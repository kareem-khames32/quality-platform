...
- **إعادة المحاولة للمشاكل الخاصة بالمكالمة:** التسجيل مش موجود لسه ← كل 15 دقيقة (8 مرات)، سيرفر الفرع مش بيرد ← كل 5 دقائق،
  صوت فاضي ← «مستبعدة» مش «فاشلة».
...
- **الخدمة:** بتقوم تلقائياً بعد إعادة تشغيل السيرفر (Delayed Start)، NSSM بيعيد تشغيلها بعد 5 ثواني لو وقعت، وWindows
  ...
  لإيقاف الـ Watchdog وقت الصيانة: اعمل ملف `maintenance.flag` جنب `WATCHDOG.bat`.
- **نسخة احتياطية يومية** لقاعدة البيانات في `data\backups` (آخر 7 أيام).
